...
float cX=2, cY=1.5;//center coordinates
...
void draw() {
  ...
  fill(0);
  stroke(0);
  float xCoor=(mouseX/((float)width/xZoom)-cX);
  float yCoor=-1*(mouseY/((float)height/yZoom)-cY);
...
void keyReleased() {
  if (key == 'z' || key == 'Z' || key=='f' || keyCode==DOWN) {
    cX=(mouseX/((float)width/xZoom)+(cX-xZoom/2));
    cY=-1*(mouseY/((float)height/yZoom)+(cY-xZoom/2));
    PImage zoomed=drawJuliaZoomed(fIter, width, height, juliaNum, cX, cY, zoom);
    imageMode(CORNER);
    image(zoomed, 0, 0);
  }
}
...
PImage drawMand(int iter, int mWidth, int mHeight) {
  ...
  for (int h=0; h<mandelb.height; h++) {
    for (int w=0; w<mandelb.width; w++) {
      ComplexNumber a=new ComplexNumber(w/(float)(mandelb.width/4)-2, h/(float)(mandelb.height/3)-1.5);
...
PImage drawJuliaZoomed(int iter, int mWidth, int mHeight, ComplexNumber c, float x1, float y1, float zoom1) {
  ...
  for (int h=0; h<mandelb.height; h++) {
    for (int w=0; w<mandelb.width; w++) {
      ComplexNumber a=new ComplexNumber(w/(float)(mandelb.width/xZoom)+(x1-xZoom/2), h/(float)(mandelb.height/yZoom)+(y1-yZoom/2));
      mandelb.pixels[h*mandelb.width+w]=color(mandelbrot(a, c, 0, iter), iter*.75, iter);
    }
  }
  mandelb.updatePixels();
